...
class BrowserSkill:
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        ...
        print(f"      [browser] goal: {goal[:90]}", flush=True)
        print(f"      [browser] start url: {url}", flush=True)
        t0 = time.time()
        ...
        print("      [browser] layer 1 (extract): static fetch + trafilatura...", flush=True)
        ...
        # ── Layer 2b: a11y ──────────────────────────────────────────────────
        print("      [browser] layer 1 insufficient -> layer 2b (a11y): "
              "driving the live page via the accessibility tree...", flush=True)
        ...
        # ── Layer 3: vision ─────────────────────────────────────────────────
        print("      [browser] layer 2b insufficient -> layer 3 (vision): "
              "screenshot + set-of-marks vision model...", flush=True)
