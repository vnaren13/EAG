...
class BaseDriver:
    ...
    async def _safe_enumerate(self) -> PageSnapshot:
        """Enumerate interactive elements, surviving an in-flight navigation.

        A click on a navigating control (e.g. a GitHub Trending filter that
        reloads the page) leaves the page mid-navigation when the NEXT turn
        starts. The raw `enumerate_interactives` then races the navigation and
        Playwright raises "Execution context was destroyed". We let the page
        settle first, and retry once if the evaluate still races."""
        for attempt in range(2):
            ...
            except Exception:                              # noqa: BLE001 - navigation race
                if attempt == 0:
    ...
    async def step(self, turn: int) -> tuple[bool, bool, str]:
        snap = await self._safe_enumerate()
    ...
    async def run(self) -> DriverResult:
        ...
        for turn in range(1, self.config.max_steps + 1):
            print(f"        [browser:{self.LAYER_NAME}] turn {turn}/{self.config.max_steps}: "
                  f"reading page + asking model for next action...", flush=True)
            done, success, note = await self.step(turn)
            last = self.steps[-1]
            acts = ", ".join(
                f"{a.get('type')}({a.get('mark', a.get('value', '')) or ''})"
                for a in (last.actions or [])
            ) or "(no action)"
            print(f"        [browser:{self.LAYER_NAME}] turn {turn}: did {acts} "
                  f"-> {last.outcome[:70]}", flush=True)
